#!/usr/bin/env Rscript
# efus_mm_london_bedroom_4month.R
#
# Fits static LME (M1-M7), AR(1) LME (M8-M10), and SAR(1,24) LME (M11-M15)
# on the London May–September subset of EFUS 2017.
#
# Models
...
arrow::write_parquet(diag_df,
  file.path(diag_dir, "london_may_sep_bedroom_diagnostics.parquet"))
...
write.csv(ranef_df,
  file.path(diag_dir, "london_may_sep_bedroom_ranef.csv"), row.names = FALSE)
...
write.csv(aic_df, file.path(diag_dir, "london_may_sep_bedroom_aic.csv"), row.names = FALSE)
...
write.csv(vc_df, file.path(diag_dir, "london_may_sep_bedroom_varcomp.csv"), row.names = FALSE)

cat("  -> diagnostics/london_may_sep_bedroom_diagnostics.parquet\n")
cat("  -> diagnostics/london_may_sep_bedroom_ranef.csv\n")
cat("  -> diagnostics/london_may_sep_bedroom_aic.csv\n")
cat("  -> diagnostics/london_may_sep_bedroom_varcomp.csv\n")
