#!/usr/bin/env Rscript
# efus_mm_london_bedroom_1month.R
#
# Fits static LME (M1-M7), AR(1) LME (M8-M10), and SAR(1,24) LME (M11-M15)
# on the London August subset of EFUS 2017.
#
# Models
...
# ── Load data ─────────────────────────────────────────────────────────────────
cat("Loading parquet ...\n")
df <- arrow::read_parquet(file.path(ROOT, "efus_indoor_outdoor_bedroom.parquet"))
df <- as.data.frame(df)
...
cat("\n")
cat(strrep("=", 108), "\n")
cat("MODEL COMPARISON: static (M1-M3)  |  static+bldg (M4-M7)  |  AR(1) (M8-M9)  |  SAR(1,24) (M11-M13)  |  SAR+bldg (M14-M15)\n")
cat(strrep("=", 108), "\n")
...
for (nm in names(all_models)) {
  ...
  if (nm %in% c("M3", "M7", "M9", "M13")) cat("  ", strrep("-", 102), "\n")
}
cat(strrep("=", 108), "\n")
...
arrow::write_parquet(diag_df,
  file.path(diag_dir, "london_august_bedroom_diagnostics.parquet"))
...
write.csv(ranef_df,
  file.path(diag_dir, "london_august_bedroom_ranef.csv"), row.names = FALSE)
...
write.csv(aic_df, file.path(diag_dir, "london_august_bedroom_aic.csv"), row.names = FALSE)
...
write.csv(vc_df, file.path(diag_dir, "london_august_bedroom_varcomp.csv"), row.names = FALSE)

cat("  -> diagnostics/london_august_bedroom_diagnostics.parquet\n")
cat("  -> diagnostics/london_august_bedroom_ranef.csv\n")
cat("  -> diagnostics/london_august_bedroom_aic.csv\n")
cat("  -> diagnostics/london_august_bedroom_varcomp.csv\n")
