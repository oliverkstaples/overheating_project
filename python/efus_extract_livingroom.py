...
using livingroom sensors only (LR).

Observations where no LR sensor is present are dropped entirely.

Output: efus_indoor_outdoor_livingroom.parquet
Columns: CaseID, hour, T_in (livingroom), Location (LR), T_out
"""
...
OUT_PATH = "/home/teaching/heating/efus_indoor_outdoor_livingroom.parquet"
...
def process_file(fpath: str) -> pd.DataFrame:
    ...
    for chunk in pd.read_csv(
        ...
        lr = chunk[chunk["Location"].isin(R_PRIORITY)].copy()

        if ext.empty or lr.empty:
            continue

        lr["rank"] = lr["Location"].map({loc: i for i, loc in enumerate(R_PRIORITY)})
        ...
        merged = lr.merge(ext_hr, on=["CaseID", "hour"], how="inner")
        parts.append(merged)
...
for i, fname in enumerate(files):
    ...
    if not part.empty:
        ...
        print(" → (no livingroom/summer data)")

print("\nConcatenating all parts...")
df = pd.concat(all_parts, ignore_index=True)

# Across-file dedup: keep highest-priority livingroom sensor per (CaseID, hour)
df["rank"] = df["Location"].map({loc: i for i, loc in enumerate(R_PRIORITY)})
...
print(f"CaseIDs with livingroom data: {df['CaseID'].nunique()}")
...
print(f"Livingroom sensors used:\n{df['Location'].value_counts()}")
